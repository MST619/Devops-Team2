...
def test_LoadGameAfterLoadGame_benchmark(monkeypatch):
    runtimings = {}
    totaltime = 0
    ...
    for x in loop:
        for i in range(x):
            ...
            timetaken = ts.test_LoadGameAfterLoadGame(monkeypatch)
    ...
    print("\nPerformance Test Results for Reload loaded gamefile after exiting loaded game Functionality:")
    print("------------------------------------------------------------------")
